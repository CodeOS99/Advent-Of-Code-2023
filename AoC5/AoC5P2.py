seeds = [[]]
# TODO       UHH YOU HAVE THE SEEDS WORKING, ITS REWRITING THE MAIN CODE-ING TIME
toSoil = []
seedToSoil = []
soilTofert = []
fertToWater = []
...
# [range(14, 79), range(13, 55)]
for each in seeds:
    if each[0][-1] > each[1][-1]:
        a = range(each[0][0], each[1][-1]+1)  # Normalize
        b = range(each[1][-1], each[0][-1]+1)  # Remaining
        toSoil.append(a)#TODO MAKE SPLITTING RANGES WORK PWEASE
        toSoil.append(b)
for seed in seeds[0]:
    toConv = int(seed)
    inp = []
